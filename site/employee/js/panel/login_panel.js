...
app.panel.Login = function(config) {
    ...
    me.init = function( config ) {
        ...
        // Create the signupContainerEl element
        signUpContainerEl = lx.createElement('DIV', {
            parent: centeredContainerEl,
            style: {
                display: 'flex',
                flexDirection: 'row',
            ...
            innerHTML: '<div>Don\'t have an account?</div>'
        });
        
        // Create signUpBtn
        signUpBtn = new lx.component.Button({
            renderTo: signUpContainerEl,
            label: 'Sign Up Here!',
        ...
        lx.sendJSON({
            ...
            onSuccess: function( responseText ) {
                ...
                if( response.loginSuccess !== true ) {
                    ...
                    if( response.hasOwnProperty('payrollAccess') && response.payrollAccess !== true ) {
                        new lx.component.Messagebox({
                            title: 'Access Denied',
                            message: 'You do not have access to the Payroll system.<br /><br />' +
                                'Please contact support if you believe you are receiving this message in error.'
                        });
                        return;
                    }
                ...
                me.fireEvent('onLogin', {
                    srcPanel: me, 
                    mustLinkProfile: response.mustLinkProfile, 
                    mustSelectCompany: response.mustSelectCompany, 
    ...
    function signUpBtnClickEventHandler() {
        ...
        var signUpPanel = new app.panel.SignUp({
            renderTo: document.body,
            
            onSignUp: function(event) {
                signUpPanel.hide();
                me.show();
                emailTxt.setValue(event.email);
                // passwordTxt.setValue(event.password);
                // loginBtnClickEventHandler();
            },
