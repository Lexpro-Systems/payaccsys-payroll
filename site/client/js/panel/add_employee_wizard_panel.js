...
app.panel.AddEmployeeWizard = function (config) {
    ...
    me.init = function (config) {
        ...
        let bweeButtonsLabelEl = lx.createElement('DIV', {
            parent: bweeButtonsContainerEl,
            style: {
                boxSizing: 'border-box',
                width: '220px',
                minWidth: '220px'
        ...
        employmentDateBtn = new lx.component.Button({
            ...
            onClick: function () {
                ...
                // Clear the custom date fields
                bweeCustomDateDate.setValue('');
                bweeCustomPaymentDayDate.setValue('');

                if (bweeCustomDateContainerEl) {
                    bweeCustomDateContainerEl.style.display = 'none';
                }
                if (paymentPeriodEndDaySelectContainerEl) {
                    paymentPeriodEndDaySelectContainerEl.style.display = 'flex';
                }
                if (paymentDaySelectContainerEl) {
                    paymentDaySelectContainerEl.style.display = 'flex';
                }
        ...
        customDateBtn = new lx.component.Button({
            ...
            onClick: function () {
                ...
                // Clear the dropdowns
                paymentPeriodEndDaySelect.setValue('');
                paymentDaySelect.setValue('');

                if (bweeCustomDateContainerEl) {
                    bweeCustomDateContainerEl.style.display = 'flex';
                }
                if (paymentPeriodEndDaySelectContainerEl) {
        ...
        bweeCustomDateContainerEl = lx.createElement('DIV', {
            parent: employmentDetailsSectionEl,
            style: {
                boxSizing: 'border-box',
                display: 'none',
        ...
        bweeCustomDateDate = new lx.component.DatePicker({
            renderTo: bweeCustomDateContainerEl,
            label: 'Custom Payment Period End Day',
            labelAlign: 'left',
            margin: '15px 0px 0px 0px',
            labelWidth: '220px',
            maxWidth: '600px',
        ...
        bweeCustomPaymentDayDate = new lx.component.DatePicker({
            renderTo: bweeCustomDateContainerEl,
        ...
        paymentDaySelectContainerEl = lx.createElement('DIV', {
            parent: employmentDetailsSectionEl,
            style: {
                boxSizing: 'border-box',
                display: 'flex',
    ...
    function wizardNextBtnClickEventHandler() {
        ...
        if (pageNum === 1) {
            ...
            let customEndDayVal = (bweeCustomDateDate && bweeCustomDateDate.getValue()) ? bweeCustomDateDate.getValue().trim() : '';
            let customPayDayVal = (bweeCustomPaymentDayDate && bweeCustomPaymentDayDate.getValue()) ? bweeCustomPaymentDayDate.getValue().trim() : '';
            let hasCustomEndDay = customEndDayVal !== '';
            let hasCustomPayDay = customPayDayVal !== '';
            let hasCustom = hasCustomEndDay || hasCustomPayDay;

            let isTwiceMonthly = paymentPeriodSelect.getValue() === "TWMO";
            let isBWEE = paymentPeriodSelect.getValue() === "BWEE";
            ...
            if (isTwiceMonthly) {
                ...
                if (!hasStandard && !hasCustom) {
                    wizardNextBtn.showWarning('Please specify either (Payment Period End Day and Payment Day) or (Custom Payment Period End Day and Custom Payment Day).');
                    return;
                }
                else if (hasStandard && hasCustom) {
                    wizardNextBtn.showWarning('Both (Payment Period End Day / Payment Day) and (Custom Payment Period End Day / Custom Payment Day) cannot be filled.');
                    return;
                }
                else if (hasStandard) {
