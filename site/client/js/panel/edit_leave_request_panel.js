...
// alert(  employeeSelect.getValue() );
        ...
        lx.sendJSON({
            ...
            onSuccess: function( responseText ) {
                ...
                // Make certain the leave balances are updated
    ...
    function loadLeaveRequest( id ) {
        ...
        lx.sendJSON({
            ...
            onSuccess: function( responseText ) {
                ...
                // Set the employee, leave type, hours and reason
                employeeSelect.setValue( 
                    response.request.employeeId, 
                    '<div class="flex-row" style="width=100%; overflow: hidden;">' + 
                        '<div class="flex-resize" style="overflow: hidden; text-overflow: ellipsis; ' +
                        'margin: 0px 5px 0px 0px;">' +
                            response.request.employeeAlias +
                        '</div>' +
                        '<div class="flex-noresize" style="overflow: hidden; text-overflow: ellipsis; ' +
                        'margin: 0px 5px 0px 0px;">' +
                        response.request.employeeCode + 
                        '</div>' +
                    '</div>'
                );
    ...
    function saveBtnClickEventHandler() {
        ...
        if( pageNum === 1 ) {
            ...
            if( employeeSelect.getValue() === null ) {
                saveBtn.showWarning('No employee selected');
                return;
            }
