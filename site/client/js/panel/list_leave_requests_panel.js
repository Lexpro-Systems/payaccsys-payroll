...
//
app.panel.ListLeaveRequests = function(config) {
    ...
    me.init = function( config ) {
        ...
        requestsGrid = new lx.component.Grid({
            ...
            columns: [
                ...
                {dataIndex: 'employeeAlias', name: 'Employee Name', minWidth: '100px', type: 'button'},
                {dataIndex: 'leaveTypeName', name: 'Leave Type', minWidth: '100px'},
    ...
    // Requests grid scroll end event handler
    ...
    function requestsGridCellClickEventHandler( event ) {
        if( requestsGrid.getColumnDataIndex( event.columnIndex ) === 'addedOn' ) {
        ...
        else if( requestsGrid.getColumnDataIndex( event.columnIndex ) === 'employeeAlias' ) {
            me.hide();
            ...
            let viewEmployeePanel = new app.panel.ViewEmployee({
                ...
                employeeName: event.record.employeeAlias,
    ...
    function requestsGridMenuItemClickEventHandler( clickEvent ) {
        if( clickEvent.value === 'view' ) {
            ...
            if( requestsGrid.getRow(clickEvent.rowIndex).statusCode !== 'PEND' ) {
                new lx.component.Messagebox({
                    title: 'Unable to Remove Leave Request',
