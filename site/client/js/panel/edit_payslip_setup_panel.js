...
//
app.panel.EditPayslipSetup = function(config) {
    ...
    function loadConfig() {
        ...
        lx.sendJSON({
            ...
            data: {
                payslipTemplateId: config.payslipTemplateId
            },
    ...
    // Function to create a configuration item
    function createConfigItems(label, name, value, type) {
        ...
        if (type === 'float') {
            ...
            let floatItem = new lx.component.Textbox({
            ...
            payslipConfigItems.push({
                el: floatItem,
                type: type,
                label: name
            });
            
            // Focus in the appropriate item
            if( label === 'Logo X' ) {
                floatItem.focus();
            }
            ...
            if( (label === 'Logo X') || (label === 'Logo Y') ) {
                // // Set the default value, if no value was specified
                // if( (value === null) || (value.trim() === '') ) {
                //     floatItem.setValue( '10' );
                // }
            ...
            else if( label === 'Logo Size (%)' ) {
                // // Set the default value, if no value was specified
                // if( (value === null) || (value.trim() === '') ) {
                //     floatItem.setValue( '100' );
                // }
            }
        }
        else if( type === 'color' ) {
            // Create the color picker item container
            let colorPickerItemContainerEl = lx.createElement('DIV', {
                parent: templateSettingsDetailsSectionEl,
            ...
            var payslipImageEl = document.createElement('IMG');
            payslipImageEl.style.margin = '15px 0px 0px 0px';
            payslipImageEl.style.width = '100%';
            payslipImageEl.style.height = '240px';
            ...
            let uploadBtn = new lx.component.Button({
                renderTo: templateSettingsDetailsSectionEl,
                label: 'Choose File',
                width: '130px',
            ...
            let uploadInputEl = document.createElement('INPUT');
                ...
                for ( var i = 0; i < files.length; i++ ) {
                    ...
                    reader.onload = ( function(aImg) {
                        return function(e) {
                            ...
                            var found = false;
                            for (var x = 0; x < payslipImages.length; x++) {
                                if (payslipImages[x].el === uploadInputEl) {
                                    payslipImages[x].file = e.target.result;
                                    payslipImages[x].imageName = name;
                                    payslipImages[x].el = uploadInputEl;
                                    found = true;
                                }
                            }
            ...
            payslipImageEl.appendChild( uploadInputEl );
    ...
    me.init = function( config ) {
        ...
        lx.createElement('DIV', {
            ...
            innerHTML: 'Edit Payslip Template Settings: ' + config.payslipTemplateName
        });
        ...
        previewPayslipBtn = new lx.component.Button({
            ...
            onClick: previewPayslipBtnEventHandler
        });
    ...
    function previewPayslipBtnEventHandler() {
        if (!payslipImageDirExists) {
            new lx.component.Messagebox({
                title: 'System Error',
                message: 'Company directory not found. Please contact support'
        ...
        for (var i = 0; i < payslipConfigItems.length; i++) {
            if(payslipConfigItems[i].type === 'float') {
                if (isNaN(payslipConfigItems[i].el.getValue())) {
                    new lx.component.Messagebox({
                        title: 'Unable to preview pdf',
                        message: payslipConfigItems[i].label + ' field needs to be a number'
                    });
                    return;
                }
                ...
                configData.push({
                    ...
                    value: payslipConfigItems[i].el.getValue(),
                    label: payslipConfigItems[i].label
                });
            }
            else if(payslipConfigItems[i].type === 'color') {
                configData.push({
                    type: payslipConfigItems[i].type,
                    value: payslipConfigItems[i].el.value,
                    label: payslipConfigItems[i].label
                });
            }
            else if(payslipConfigItems[i].type === 'image') {
                if( payslipImages !== null ) {
                    for (var x = 0; x < payslipImages.length; x++) {
                        if(payslipImages[x].el === payslipConfigItems[i].el) {
                            configData.push({
                                type: payslipConfigItems[i].type,
                                value: payslipImages[x].file,
                                label: payslipConfigItems[i].label
                            });
                        }
                    }
    ...
    function saveBtnClickEventHandler() {
        ...
        lx.sendJSON({
            url: 'exec.php?c=PayslipConfig&fn=updatePayslipTemplatesConfig',
            data: {
                payslipTemplateId: config.payslipTemplateId,
                configData: configData
            },
