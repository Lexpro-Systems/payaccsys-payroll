...
app.panel.ListPayruns = function (config) {
    ...
    function importPayrunBtnClickEventHandler() {
        // Create a modal window
        var importPayrunModal = new lx.component.ModalWindow({
            margin: '20px',
            maxHeight: '100%',
            maxWidth: '980px'
        });
        ...
        var importPayrunPanel = new app.panel.ImportPayrun({
            renderTo: importPayrunModal.getContainer(),
            show: true,
        ...
        // Add destroy event listener to modal to destroy the contained panel.
        importPayrunModal.addEventListener('destroy', function () {
            importPayrunPanel.destroy();
        });

        // Create a route entry for the panel
        let state = {
            modal: importPayrunModal
        };
        app.route.pushState(state, function (state) {
            state.modal.destroy();
        });
        ...
        // Show the modal window and focus on the panel
        importPayrunModal.show();
        importPayrunPanel.focus();
    }
