...
app.panel.PayrunReports = function (config) {
    ...
    var ytdReportSectionEl = null;
    var ytdReportBtn = null;

    var payslipItemsSectionEl = null;
    var payslipItemsBtn = null;

    var payslipItemsBroaderSectionEl = null;
    var payslipItemsBroaderBtn = null;
    ...
    me.init = function (config) {
        ...
        // Create the ytdReportSectionEl element
        ytdReportSectionEl = lx.createElement('DIV', {
            parent: contentContainerEl,
            style: {
                backgroundColor: '#FFFFFF',
                borderStyle: 'solid',
            ...
            innerHTML:
                '<div><div style="font-size: 18px;">Year-to-Date</div>' +
                '<div style="font-size: 12px; margin: 10px 0px 0px 0px;">' +
                'Show the year to date report details.' +
                '</div></div>'
        });

        // Create the ytdReportBtn component
        ytdReportBtn = new lx.component.Button({
            renderTo: ytdReportSectionEl,
            label: 'Open',
            width: '120px',
            margin: '0px 0px 0px 30px',

            onClick: ytdReportBtnClickEventHandler
        });
        ...
        //
        // PAYSLIP ITEMS( Cleaner overview) SECTION
        //

        // Create the payslipItemsSectionEl element
        payslipItemsBroaderSectionEl = lx.createElement('DIV', {
            parent: contentContainerEl,
            style: {
                backgroundColor: '#FFFFFF',
                borderStyle: 'solid',
        ...
        //   payslipItemsBtn = new lx.component.Button({
        //     renderTo: payslipItemsSectionEl,
        payslipItemsBroaderBtn = new lx.component.Button({
            renderTo: payslipItemsBroaderSectionEl,
            label: 'Open',
            width: '120px',
            margin: '0px 0px 0px 30px',
            ...
            onClick: payslipItemsBroaderBtnClickEventHandler
        });
    ...
    // ytdReportBtn click event handler
    function ytdReportBtnClickEventHandler() {
        config.parentPanel.hide();

        var ytdReportPanel = new app.panel.YearToDateReport({
            renderTo: app.mainPanel.getContainer(),
            show: true
        });
        ytdReportPanel.focus();

        var panelState = {
            previousPanel: config.parentPanel,
            panel: ytdReportPanel
        };
    ...
    function payslipItemsBroaderBtnClickEventHandler() {
        config.parentPanel.hide();

        var payslipBroaderItemsPanel = new app.panel.PayslipBroaderItemsReport({
            renderTo: app.mainPanel.getContainer(),
            show: true
        });
        payslipBroaderItemsPanel.focus();

        var panelState = {
            previousPanel: config.parentPanel,
            panel: payslipBroaderItemsPanel
        };
