...
//
app.panel.EditRecurringPayslipConfigItem = function(config) {
    ...
    var importHoursContainer = null;
    var importHoursCheck = null;
    var partOfNettPayContainer = null;
    var partOfNettPayCheck = null;
    ...
    function loadPayslip() {
        lx.sendJSON({
            ...
            onSuccess: function( responseText ) {
                ...
                itemAutoCheck.setValue(response.payslip.autoCalculate);
                itemAutoCheckChangeEventHandler();
                
                // Set auto calculate depending on the selected item type.
                if( response.payslip.autoCalculate === true ) {
                ...
                else if( response.payslip.payslipItemUnitCode === 'PDAY') {
                    itemAmountTxt.setLabel('Daily Rate');
                    lx.applyStyle(importHoursContainer, {display: 'none'});
                }
                else if( response.payslip.payslipItemUnitCode === 'PKIL') {
                    itemAmountTxt.setLabel('Rate Per Kilometer');
                    lx.applyStyle(importHoursContainer, {display: 'none'});
                }
                else if( response.payslip.payslipItemUnitCode === 'PERC') {
                    itemAmountTxt.setLabel('Percentage');
                    lx.applyStyle(importHoursContainer, {display: 'none'});
                }
        
                // Is it a fringe benefit?
                if(response.payslip.payslipCategoryCode === 'FBEN' ) {
    ...
    me.init = function( config ) {
        ...
        // Create item section
        importHoursContainer = lx.createElement('DIV', {
        ...
        importHoursCheck = new lx.component.Checkbox({
        ...
        // Create item section
        partOfNettPayContainer = lx.createElement('DIV', {
            parent: itemSectionEl,
            style: {
                display: 'none',
                margin: '20px 0px 0px 0px'
        ...
        partOfNettPayCheck = new lx.component.Checkbox({
    ...
    function itemTypeSelectChangeEventHandler() {
        ...
        partOfNettPayCheck.setValue( selectedItemType.includeInNettPay );
        ...
        // Update the amount label depending on unit
        if( selectedItemType.unit.code === 'FIXE' )  {
            itemAmountTxt.setLabel('Amount');
            lx.applyStyle(importHoursContainer, {display: 'none'});
        }
        else if( selectedItemType.unit.code === 'PHOU')  {
            itemAmountTxt.setLabel('Hourly Rate');
            // Is it an hourly wage?
            if( selectedItemType.code == '1001' ) {
                lx.applyStyle(importHoursContainer, {display: 'flex'});
            }
            else {
                lx.applyStyle(importHoursContainer, {display: 'none'});
        ...
        else if( selectedItemType.unit.code === 'PDAY')  {
            itemAmountTxt.setLabel('Daily Rate');
            lx.applyStyle(importHoursContainer, {display: 'none'});
        }
        else if( selectedItemType.unit.code === 'PKIL')  {
            itemAmountTxt.setLabel('Rate Per Kilometer');
            lx.applyStyle(importHoursContainer, {display: 'none'});
        }
        else if( selectedItemType.unit.code === 'PERC')  {
            itemAmountTxt.setLabel('Percentage');
            lx.applyStyle(importHoursContainer, {display: 'none'});
        }
