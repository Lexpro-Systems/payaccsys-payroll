...
//
app.panel.EditRetirementFund = function(config) {
    ...
    // Funtion to load retirement fund details
    function loadRetirementFund() {
        lx.sendJSON({
            url: 'exec.php?c=ProvidentFund&fn=get',
            data: {
                providentFundId: config.providentFundId
            },
            onSuccess: function( responseText ) {
                ...
                if(response.providentFund.providentFundCalculationTypeCode === 'FIXE') {
                ...
                nameTxt.setValue(response.providentFund.name);
                providentFundCalculationTypes.setValue(response.providentFund.providentFundCalculationTypeCode);
                employeeAmountTxt.setValue( response.providentFund.employeeAmount !== '' ? formatNumerical(response.providentFund.employeeAmount, 2) : '0.00' );
                employerAmountTxt.setValue( response.providentFund.employerAmount !== '' ? formatNumerical(response.providentFund.employerAmount, 2) : '0.00' );
            }
        });
    }
    ...
    me.init = function( config ) {
        ...
        lx.createElement('DIV', {
            ...
            innerHTML: 'Edit Retirement Fund'
        });
    ...
    function saveBtnClickEventHandler() {
        ...
        if( nameTxt.getValue() === '' ) {
            saveBtn.showWarning('Please enter a fund name.');
            return;
        }
        
        if( employeeAmountTxt.getValue() === '' ) {
            if(providentFundCalculationTypes.getValue() === 'FIXE') {
                saveBtn.showWarning('Please enter an employee amount.');
                return;
            }
            else {
                saveBtn.showWarning('Please enter an employee percentage.');
                return;
            }
        }
        ...
        if( employerAmountTxt.getValue() === '' ) {
            if(providentFundCalculationTypes.getValue() === 'FIXE') {
                saveBtn.showWarning('Please enter an employer amount.');
                return;
            }
            else {
                saveBtn.showWarning('Please enter an employer percentage.');
                return;
            }
        }
        ...
        saveBtn.disable();
        lx.sendJSON({
            url: 'exec.php?c=ProvidentFund&fn=update',
            data: {
                providentFundId: config.providentFundId,
