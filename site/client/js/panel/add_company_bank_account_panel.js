...
//
app.panel.AddCompanyBankAccount = function(config) {
    ...
    me.init = function( config ) {
        ...
        // Create the addBtnTooltip component.
        addBtnTooltip = new lx.component.Tooltip({
            renderTo: addBtnContainerEl,
            maxWidth: '300px',
            arrowOffset: 'center',
            
            onBlur: addBtnTooltipBlurEventHandler
        });
    ...
    // addBtnTooltip blur event handler
    function addBtnTooltipBlurEventHandler( event ) {
    ...
    function addBtnClickEventHandler() {
        ...
        if(bankNameSelect.getValue() === null) {
            addBtn.showWarning('Your bank name can not be empty');
            return;
        }
        
        if(accountNumberTxt.getValue() === '') {
            addBtn.showWarning('Your bank account number can not be empty');
            return;
        }
        
        if(branchCodeTxt.getValue() === '') {
            addBtn.showWarning('Your branch code can not be empty');
            return;
        }
        ...
        lx.sendJSON({
            url: 'exec.php?c=Company&fn=addBank',
            data: {
            ...
            onSuccess: function( responseText ) {
                ...
                me.fireEvent('add', {
                    srcPanel: me, 
                    companyBankAccountId: response.departmentId, 
                    accountNumber: accountNumberTxt.getValue().trim(),
                    financialInstitutionName: bankNameSelect.getText().trim()
                });
            }
        });
