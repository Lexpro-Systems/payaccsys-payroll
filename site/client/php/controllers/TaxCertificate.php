<?php
    ...
    //
    // TAX CERTIFICATE CONTROLLER CLASS
    //
    ...
    class TaxCertificate extends Controller {
        ...
        // Function to Get a list of employees for whom tax certificates should be generated in a given tax year
        // as well as details about the tax certificates if it has been generated already
        //
        // Required Parameters
        //  taxYear             The tax year of the IRP5 certificate details
        //  typeCode            Annual (ANNU) or interim (INTE)
        //
        // Optional Parameters
        //  None
        public function getEmployeeList($data, $user, $db) {
            ...
            // Get details of all employees who should get tax certificates of the specified type
            ...
            $sqlQuery = 
                'SELECT DISTINCT ' .
                    ...
                    'tax_certificates.id AS tax_certificate_id, ' .
                    'tax_certificates.number AS number, ' .
                ...
                'LEFT JOIN ' .
                    'tax_certificates ON tax_certificates.employee_id = employees.id ' .
                'LEFT JOIN ' .
            ...
            while( $sqlRow = $sqlResult->fetchAssociative() ) {
                $employees[] = [
                    ...
                    'taxCertificateId' => $sqlRow['tax_certificate_id'],
                    'number' => $sqlRow['number'],
                    'taxYear' => $sqlRow['sars_year'],
                    'periodCode' => $sqlRow['period_code'],
                    'periodName' => $sqlRow['period_name'],
                    'typeCode' => $sqlRow['tax_certificate_type_code'],
                    'typeName' => $sqlRow['tax_certificate_type_name'],
        ...
        // Function to send tax certificates via email
        //
        // Required Parameters
        //  taxCertificates             An array describing the taxCertificates to send
        // 
        //  taxCertificates = [
        //      'id',                   // The tax certificate id
        //      'emailAddress'          // The email address to send the tax certificates to
        //  ]
        //
        public function send($data, $user, $db) {
            ...
            // Validate data.
            $validationResult = Json::validate($data, [
                'taxCertificates' => ['type' => Json::TYPE_ARRAY, 'required' => true, 'nullable' => false, 'rules' => [
                    ['type' => Json::TYPE_OBJECT, 'required' => true, 'nullable' => false, 'rules' => [
                        'id' => ['type' => Json::TYPE_INT, 'required' => true, 'nullable' => false],
                        'emailAddress' => ['type' => Json::TYPE_NON_EMPTY_STRING, 'required' => false, 'nullable' => false]
                    ]]
                ]]
            ]);
            ...
            foreach( $data['taxCertificates'] as $taxCertificate ) {
        ...
        // Function to download an tax certificate certfiicate in pdf format
        //
        // Required Parameters
        //  taxCertificateId               The id of the irp5 to download
        //
        ...
        // Function to update the self service access of the given tax certificates
        //
        // Required Parameters
        //  taxCertificates                 An array describing the tax certificates
        //
        //  taxCertificates = [
        //      'id',                       // The tax certificate id
        //      'selfServiceAccess'         // Whether the tax certificate is accessible in the self service portal (true or false)
        //  ]
        //
        // Optional Parameters
        //  None
        public function updateSelfServiceAccess($data, $user, $db) {
            // Set content type header
            header('Content-Type: application/json');
            ...
            $validationResult = Json::validate($data, [
                ...
                'taxCertificates' => ['type' => Json::TYPE_ARRAY, 'required' => true, 'nullable' => false, 'rules' => [
                    ['type' => Json::TYPE_OBJECT, 'required' => true, 'nullable' => false, 'rules' => [
                        'id' => ['type' => Json::TYPE_INT, 'required' => true, 'nullable' => false],
                        'selfServiceAccess' => ['type' => Json::TYPE_BOOL, 'required' => true, 'nullable' => false]
                    ]]
                ]]
            ]);
            if( $validationResult !== true ) {
            ...
            // For each tax certificate
            foreach( $data['taxCertificates'] as $certificate ) {
                // Mark the self-service access for the specified certificate(s)
                $sqlQuery = 'UPDATE tax_certificates SET self_service_access = $1 WHERE id = $2;';
                $sqlResult = $db->paramQuery($sqlQuery, [
                    $certificate['selfServiceAccess'],
                    $certificate['id']
                ]);
                if( !$sqlResult->isValid() ) {
                    echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
