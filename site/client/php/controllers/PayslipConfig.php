<?php
    ...
    class PayslipConfig extends Controller {
        ...
        // Function to get a payslip media resource
        //
        // Get Parameters
        //  name             The name of the resource to get.
        public function getPayslipImage($data, $user, $db) {
            ...
            $resourcePath = realpath(CONF_CLIENT_DIR . $sqlRow['value'] .'/payslip_images/'. $_GET['name']);
        ...
        // Function to list payslips templates
        //
        // Required Parameters
        //  None
        //
        // Optional Parameters
        // None
        public function updatePayslipTemplatesConfig($data, $user, $db) {
            header('Content-Type: application/json');
            ...
            $validationResult = Json::validate($data, [
                // Required parameters
                'payslipTemplateId' => ['type' => Json::TYPE_INT, 'required' => true, 'nullable' => false]
            ]);
            ...
            for ($i=0; $i < count($data['configData']); $i++) {
                ...
                if ($data['configData'][$i]['type'] === 'image') {
                    ...
                    $sqlQuery = 'SELECT value FROM payslip_template_config WHERE name = $1;';
                ...
                $sqlQuery = 'SELECT id, name, value FROM payslip_template_config WHERE payslip_template_id = $1 AND name = $2';
                $sqlResult = $db->paramQuery($sqlQuery, [
                    $data['payslipTemplateId'],
                ...
                if( $sqlResult->getRowCount() === 0 ) {
                    if ($value !== '') {
                        $sqlQuery =  'INSERT INTO payslip_template_config (payslip_template_id, name, value) VALUES ($1, $2, $3);';
                        $sqlResult = $db->paramQuery($sqlQuery, [
                            $data['payslipTemplateId'],
                            $data['configData'][$i]['label'],
                            $value
                    ...
                    $sqlQuery =  'UPDATE payslip_template_config SET value = $1 WHERE payslip_template_id = $2 AND name = $3;';
                    $sqlResult = $db->paramQuery($sqlQuery, [
                        $value,
                        $data['payslipTemplateId'],
                        $data['configData'][$i]['label']
        ...
        // Optional Parameters
        // None
        public function getPayslipTemplatesConfig($data, $user, $db) {
            ...
            // NOTE:
            //
            // The 'client_data_dir' is created in the CONF_CLIENT_DIR folder when the bookset is created and
            // consists of the schema name with an underscore and 6 random characters at the end.
            ...
            $sqlQuery = 'SELECT id, name, description FROM payslip_templates WHERE id = $1';
            $sqlResult = $db->paramQuery($sqlQuery, [$data['payslipTemplateId']]);
            if( !$sqlResult->isValid() ) {
                echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
                return false;
            }
            ...
            $sqlQuery = 'SELECT id, payslip_template_id, name, value FROM payslip_template_config WHERE payslip_template_id = $1';
            $sqlResult = $db->paramQuery($sqlQuery, [$data['payslipTemplateId']]);
            if( !$sqlResult->isValid() ) {
                echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
                return false;
            ...
            if( $clientDataDir !== NULL ) {
                $clientDataDir = str_replace('\\', '/', $clientDataDir);
                $clientDataDir = str_replace('//', '/', $clientDataDir);
            }
            
// echo( json_encode(['ok' => false, 'error' => (CONF_CLIENT_DIR . $clientDataDir .'/')]) );
// return false;
                    ...
                    if ($printerTemplateConfig[$i]['name'] === $savedTemplateConfig[$b]['name']) {
                        ...
                        if ($printerTemplateConfig[$i]['type'] === 'image') {
                            ...
                            if ($savedTemplateConfig[$b]['value'] !== '') {
                                ...
                                $filePath = CONF_CLIENT_DIR . $clientDataDir .'/payslip_images/' . $savedTemplateConfig[$b]['value'] .'.png';
                                ...
                                if (file_exists($filePath)) {
                                    $value = 'exec.php?c=PayslipConfig&fn=getPayslipImage&name=' . urlencode($savedTemplateConfig[$b]['value'] .'.png') . '&v=' . filemtime($filePath);
                                }
        ...
        // Optional Parameters
        // None
        public function getPayslipTemplatesList($data, $user, $db) {
            
            $sqlQuery = 'SELECT value FROM config WHERE name = \'payslip_template\'';
            $sqlResult = $db->paramQuery($sqlQuery, []);
            if( !$sqlResult->isValid() ) {
                echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
                return false;
            ...
            $sqlQuery = 'SELECT id, name, description FROM payslip_templates ORDER BY name ASC';
            $sqlResult = $db->paramQuery($sqlQuery, []);
            if( !$sqlResult->isValid() ) {
                echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
                return false;
        ...
        //
        public function downloadPayslipPreview($data, $user, $db) {
            header('Content-Type: application/json');
            ...
            for ($i = 0; $i < count($savedImagesList); $i++) {
                $sqlQuery = 'SELECT name FROM payslip_template_config WHERE value = $1 AND payslip_template_id = $2;';
                $sqlResult = $db->paramQuery($sqlQuery, [substr($savedImagesList[$i], 0, strpos($savedImagesList[$i], '.')), $data['payslipTemplateId']]);
            ...
            $printer->setCompanyName('Company Name');
            $printer->setCompanyAddress('Company Address Line 1', 'Company Address Line 2', 'Company Address Line 3', 'Company Address Line 4');
            $printer->setCompanyTel('Company Tel');
            $printer->setCompanyFaxNumber('Company Fax');
            $printer->setCompanyEmail('Company Email');
            // $printer->setCompanyLogo('Company Logo');
            
            $printer->setEmployeeFullName('Full Names');
            $printer->setEmployeeAlias('Alias');
            $printer->setEmployeeCode('Code');
            $printer->setEmployeeDepartment('Department');
            $printer->setEmployeePosition('Position');
            $printer->setEmployeeEmploymentStart('Start Date');
            $printer->setEmployeeEmail('Email Address');
            $printer->setEmployeeAddress('Postal Address Line 1', 'Postal Address Line 2', 'Postal Address Line 3', 'Postal Address Code');
            $printer->setEmployeeCell('Cell Number');
            $printer->setEmployeeIdNumber('Id Number');
            
            $printer->setEmployeeBankName('Bank Account Type Name');
            
            $printer->setEmployeeAccountNumber('Account Number');
            
            $printer->setEmployeeBankCode('Branch Code');
            
            $printer->setEmployeePeriod('Period');
            $printer->setPayslipToDate('To Date');
            $printer->setPayslipToDate('From Date');
            
            $amount = 10;
            for ($i=0; $i < 2; $i++) { 
                
                $printer->addPayslipItem('Income', 'Description', $amount + $i, false);
                $printer->addPayslipItem('Deductions', 'Description', $amount + $i, false);
                $printer->addPayslipItem('Company Contributions', 'Description', $amount + $i, false);
                $printer->addPayslipItem('Fringe Benefits', 'Description', $amount + $i, false);
                $printer->addPayslipItem('Allowances', 'Description', $amount + $i, false);
                $printer->addLeaveItem('Leave Type', 0, 0, 0,0, 'days');
            }
        ...
        private function processImage( $filepath) {
            ...
            imagecopyresized($resizedImage, $formattedImage, 0, 0, 0, 0, (int)$newWidth, (int)$newHeight, (int)$width, (int)$height);
        ...
        private function formatFile($filepath) {
            ...
            switch ($type) { 
                ...
                case 3 : 
                    // NOTE:
                    //
                    // Sometimes the imageCreateFromPng() function gives the following warning:
                    //
                    //     Warning: imagecreatefrompng(): gd-png: libpng warning: iCCP: known incorrect sRGB profile...
                    //
                    // The ob_start() and ob_end_clean() statements have been added to prevent the warning from being
                    // sent to the browser so that the return result will be able to be parsed
                    //
                    ob_start();
