...
function calculateLoanInstalment($interestTypeCode, $paymentPeriodTypeCode, $capitilizationPeriodTypeCode, float $principalAmount, int $numPayments, float $annualInterestRate): float
{
    ...
    } else if ($paymentPeriodTypeCode === 'TWMO') {
        $totalYears = $numPayments / 24;
...
# This the updated getBiWeeklyPayslipPeriod function that includes custom dates but is no longer called, kept as a backup.
// function getBiWeeklyPayslipPeriod(DateTime $date,DateTime $anchorDate): int {

//     # NOTE: This is a simplified version of getBiWeeklyPayslipPeriod

//     $endDate = new DateTime($date->format('Y-m-d'));
//     $taxYearStart = new DateTime($endDate->format('Y') . '-03-01');

//     # January and February belong to the previous SARS tax year
...
//     # Move backwards until we reach the first BWEE cycle that can belong to this SARS year
...
//         if ($periodEnd >= $taxYearStart && $periodEnd <= $endDate) {
//             $period++;
//         }
 ...
 *     1 to 27 = calendar day
 *     0       = last calendar day of the month
 *
 * A period crosses a month boundary when its start day is numerically
 * greater than its end day, except when the end day is 0.
 *
 * Examples:
 *     25 to 10  = 25 August to 10 September
 *     11 to 24  = 11 September to 24 September
 *     16 to 0   = 16 September to 30 September
 *
