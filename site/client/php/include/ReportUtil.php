...
// @param $user             The user object
// @param $db               The database object
function gLSD($data, $user, $db)
{
    /********************************
     Retrieves the LEAVE ACTIONS
     ********************************/
     ...
     ********************************/
    $sqlQuery = 'SELECT name, leave_unit_code FROM leave_types';
    ...
    while ($sqlRow = $sqlResult->fetchAssociative()) {
        ...
        $leaveTypes[] = [
            ...
            'amount' =>  $leaveTypeUnit
        ];
    }

    /***************************************
     Gets all employee's & their leaveTypes
     ****************************************/
...
function getEmployeeLeaveData($data, $user, $db)
{
    ...
    array_multisort(
        array_column($employeeLeave, 'leaveType'),
        SORT_ASC,
        array_column($employeeLeave, 'closingBalance'),
        SORT_ASC,
        $employeeLeave
    );
...
function getUifData($data, $user, $db)
{
    ...
    $sqlQuery =
        ...
        'employees.id_number AS id_numba, ' .
        'employees.id_number AS wage_type, ' .
        'employees.id_number AS wage_tariff, ' .
        'employees.id_number AS total_earnings ' .
    ...
    while ($sqlRow = $sqlResult->fetchAssociative()) {
        ...
        if ($employmentEndDate !== '') {
            ...
            if ($sqlRow['dismissal_reason'] === 'ABSC') {
                ...
                $employmentStatus = 12;
            } else if ($sqlRow['dismissal_reason'] === 'MATE') {
                $employmentStatus = 9;
            }
        }
...
function getPayslipItemsData($data, $user, $db)
{
    ...
    $sqlQuery =
        ...
        'employees.employment_end_date AS employee_employment_end_date,' .
        'CAST(COALESCE(income_details.total, 0) AS FLOAT) AS total_income, ' .
        'CAST(COALESCE(deduction_details.total, 0) AS FLOAT) AS total_deductions, ' .
        'CAST(COALESCE(contribution_details.total, 0) AS FLOAT) AS total_contributions, ' .
        'CAST(COALESCE(fringe_benefit_details.total, 0) AS FLOAT) AS total_fringe_benefits, ' .
    ...
    $totalNetIncome = 0.00;
    while ($sqlRow = $sqlResult->fetchAssociative()) {
        // Check if employee was dismissed before start date
        if (($data['startDate'] != null) && ($data['startDate'] != '')) {
            if (($sqlRow['employee_employment_end_date'] != null && $sqlRow['employee_employment_end_date'] != '') && $sqlRow['employee_employment_end_date'] < $data['startDate']) continue;
        }
        if (isZeroPayroll($sqlRow)) {
            continue;
        }
        $empid = $sqlRow['employee_id'];
        $item = getPayslipitems($empid, $filterClause, $filterValues, $db);
        // Set the employee details
        $employees[] = [
            'employeeCode' => $sqlRow['employee_code'],
            'employeeAlias' => $sqlRow['employee_alias'],
            ...
            'employee_employment_end_date' => $sqlRow['employee_employment_end_date'],
            'totalEmployerUif' => $sqlRow['total_employer_uif'],
            'totalSdl' => $sqlRow['total_sdl'],
            'totalContributions' => $sqlRow['total_contributions'],
            'totalFringeBenefits' => $sqlRow['total_fringe_benefits'],
...
function getPayslipitems($empId, $filterClause, $filterValues, $db)
{

    $params = $filterValues;   // clone it
    $params[] = $empId;

    $filterCount = count($params);
    $param = '$' . $filterCount;
    ...
    $sqlItemQuery =
        ...
        'WHERE payslips.status_code = \'ACTI\' ' .
        $filterClause .
        ' AND payslip_items.id IS NOT NULL ' .
        ' AND payslips.employee_id = ' . $param .
        ') ' .
        'SELECT * FROM payslip_details';

    $sqlItemResult = $db->paramQuery($sqlItemQuery, $params);
    ...
    while ($row = $sqlItemResult->fetchAssociative()) {

        $key = $row['payslip_category_code'] . '|' . $row['description'];

        if (!isset($items[$key])) {
            $items[$key] = [
                'payslipCategoryCode' => $row['payslip_category_code'],
                'description' => $row['description'],
                'total' => 0
            ];
        }

        $items[$key]['total'] += (float)$row['total'];
    }

    return $items;
}
function isZeroPayroll($row): bool
{
    $fields = [
        'total_employer_uif',
        'total_sdl',
...
function getUI19PdfDetailsData($data, $user, $db)
{
    $sqlParams = [];
    ...
    if (isset($data['employeeId']) && $data['employeeId'] !== '') {
        ...
        $whereClause = $whereClause . ' employees.id = $' . count($sqlParams);
    }
    ...
    $sqlQuery =
        ...
        'employees.employment_start_date,' .
        'employees.employment_end_date,' .
        'employment_history.dismissal_reason,' .
        'employees.work_address_unit,' .
        'employees.work_address_complex,' .
        'employees.work_address_street,' .
        'employees.work_address_suburb,' .
        ...
        'employees ' .
        'LEFT JOIN ' .
        'employment_history ON employees.id = employment_history.employee_id ' .
        $whereClause;
    $sqlResult = $db->paramQuery($sqlQuery, $sqlParams);
    if (!$sqlResult->isValid()) {
        return (['ok' => false, 'error' => 'Database error.']);
    ...
    while ($sqlRow = $sqlResult->fetchAssociative()) {
        ...
        $employees[] = [
            ...
            "disCode" => $sqlRow['dismissal_reason'],
            'workUnit' => $sqlRow['work_address_unit'],
            'workComplex' => $sqlRow['work_address_complex'],
            'workStreet' => $sqlRow['work_address_street'],
            'workSuburb' => $sqlRow['work_address_suburb'],
            'workCity' => $sqlRow['work_address_city'],
            'workPostalCode' => $sqlRow['work_address_postal_code'],
        ];
    }
    ...
    $CompanyResult = $db->paramQuery($CompsqlQuery, []);
    if (!$CompanyResult->isValid()) {
        return (['ok' => false, 'error' => 'Database error.']);
    }
    $companyInfo = [];
    while ($sqlRow = $CompanyResult->fetchAssociative()) {
        $companyInfo[] = [
            'payeRefNumber' => $sqlRow['paye_reference_number'],
            'companyName' => $sqlRow['name'],
            'physicalUnit' => $sqlRow['physical_address_unit'],
            'physicalComplex' => $sqlRow['physical_address_complex'],
            'physicalStreet' => $sqlRow['physical_address_street'],
            'physicalSuburb' => $sqlRow['physical_address_suburb'],
            'physicalCity' => $sqlRow['physical_address_city'],
            'physicalPostalCode' => $sqlRow['physical_address_postal_code'],
            'postalLine1' => $sqlRow['postal_address_line_1'],
            'postalLine2' => $sqlRow['postal_address_line_2'],
            'postalLine3' => $sqlRow['postal_address_line_3'],
            'postalCode'  => $sqlRow['postal_address_code'],
            ...
            'Refnumber'  => $sqlRow['uif_registration_number'],
        ];
    };
    ...
    foreach ($employees as &$employee) {
        foreach ($company as $key => $value) {
            $employee[$key] = $value;   // attach each company field
        }
    }
    unset($employee);
    // -------------------------
    // PAYRUN QUERY (KEY VALUE)
    // -------------------------
    $payrunQuery =
        'SELECT ' .
        ...
        'AND description IN ' .
        "('Salary','Commission','Daily Wage','Hourly Wage','Overtime','Employee UIF Contribution')";

    $payrunResult = $db->paramQuery($payrunQuery, $sqlParams);

    if (!$payrunResult->isValid()) {
        return ['ok' => false, 'error' => 'Database error.'];
    }

    // store description => income
    $incomeValues = [];
    $uifFound = 0;
    while ($row = $payrunResult->fetchAssociative()) {
        //$incomeValues[$row['description']] = $row['amount'];
        if ($row['description'] === 'Employee UIF Contribution') {
            $uifFound = 1;
        } else {
            $incomeValues[$row['description']] = $row['amount'];
        }
    }
    $incomeValues['Employee UIF Contribution'] = $uifFound;
    // attach income values same way as company fields
    foreach ($employees as &$employee) {
        foreach ($incomeValues as $desc => $value) {
            $employee[$desc] = $value;
        }
    }
    unset($employee);
