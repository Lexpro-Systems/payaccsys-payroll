<?php
    ...
    class User extends Controller {
        ...
        public function login($data, $user, $db) : bool {
            ...
            $sqlQuery =
                'SELECT ' .
                    'user_id, company_id, is_default, ' .
                    'companies.database_name, companies.database_schema, companies.database_host ' . 
                'FROM ' .
                    'user_company_access ' .
                'LEFT JOIN ' .
            ...
            if( $companyId !== null ) {
                ...
                $sqlQuery = 'SELECT user_right_code FROM user_company_rights WHERE user_right_code = \'AAPO\' AND user_id = $1 AND company_id = $2 LIMIT 1;';
        ...
        public function switchCompany($data, $user, $db) : bool {
            ...
            // Check if the user has the right to access the attendance register portal
            $sqlQuery = 'SELECT user_right_code FROM user_company_rights WHERE user_right_code = \'AAPO\' AND user_id = $1 AND company_id = $2 LIMIT 1;';
            $sqlResult = $db->paramQuery($sqlQuery, [
                $user['id'], 
                $data['companyId']
            ...
            // Was the right to access the company attendance register not found?
            if( $sqlResult->getRowCount() !== 1 ) {
                echo( json_encode(['ok' => false, 'error' => 'Company attendance register access denied.']) );
                return false;
            }
