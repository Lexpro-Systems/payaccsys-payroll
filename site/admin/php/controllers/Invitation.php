<?php
    ...
    class Invitation extends Controller {
        ...
        // Function to get a list of invitations
        //
        // Required Parameters
        //
        // Optional Parameters
        ...
        public function getList($data, $user, $db) : bool {
            ...
            $validationResult = Json::validate($data, [
                ...
                'invitationStatusCode' => ['type' => Json::TYPE_STRING, 'required' => false, 'nullable' => true]
            ]);
            ...
            if( isset($data['searchString']) && $data['searchString'] !== '' ) {
                ...
                $whereClause = ' WHERE ';
                $whereClause = $whereClause . ' (user_company_invitations.invitee_name ILIKE \'%\' || $' . count($sqlParams) . ' || \'%\' ) OR ';
                $whereClause = $whereClause . ' (companies.name ILIKE \'%\' || $' . count($sqlParams) . ' || \'%\' ) OR ';
                $whereClause = $whereClause . ' (user_company_invitations.invitee_email_address ILIKE \'%\' || $' . count($sqlParams) . ' || \'%\' ) ';
            }
            
            // Was a leave request status code filter specified?
            if( $data['invitationStatusCode'] !== null ) {
                if( $whereClause !== '' ) {
                ...
                $sqlParams[] = $data['invitationStatusCode'];
                $whereClause = $whereClause . 'user_company_invitations.status_code = $' . count($sqlParams) . ' ';
            }
            ...
            if( isset($data['sortList']) ) {
                ...
                foreach( $data['sortList'] AS $sortItem ) {
                    ...
                    switch( $sortItem['dataIndex'] ) {
                        case 'name':
                            $column = 'user_company_invitations.invitee_name';
                            break;
                        case 'emailAddress':
                            $column = 'user_company_invitations.invitee_email_address';
                            break;
                        case 'companyName':
                            $column = 'companies.name';
                            break;
                        case 'statusName':
                            $column = 'user_company_invitation_status_types.name';
                            break;
                        case 'expiresOn':
                            $column = 'user_company_invitations.expires_on';
                            break;
                        default:
            ...
            $sqlQuery =
                'SELECT DISTINCT ' .
                    ...
                    'user_company_invitations.expires_on, ' .
                ...
                'LEFT JOIN ' .
                    'companies ON companies.id = user_company_invitations.company_id ' . 
                'LEFT JOIN ' .
                ...
                $sortClause . ' ' . 
                // 'ORDER BY ' .
                //     'user_company_invitations.invitee_name ' . $data['sortOrder'] . ' ' . 
                $limitOffset;
        ...
        public function accept($data, $user, $db) {
            ...
            $sqlQuery = 'SELECT id FROM users WHERE LOWER(email_address) = LOWER($1);';
            ...
            $sqlQuery = 'SELECT id FROM user_company_access WHERE company_id = $1 AND user_id = $2;';
        ...
        public function getInvitationById($data, $user, $db) {
            ...
            if( $sqlResult->getRowCount() === 1 ) {
                ...
                $invitation = [
                    ...
                    'url' => CONF_CLIENT_URL . '/index.html?invitation=' . $sqlRow['code'],
                    'status' => [
                        'code' => $sqlRow['status_code'],
                        'name' => $sqlRow['status_name']
                    ],
        ...
        public function resend($data, $user, $db) {
            ...
            $sqlQuery =
                'SELECT ' .
                    'company_id, companies.name AS company_name, invitee_email_address, invitee_name, sent_by_user_id, users.name AS sender_name, user_company_invitations.code AS invite_code, ' .
                    'sent_on, expires_on, status_code, user_company_invitation_status_types.name AS status_name, ' .
                    'invitee_users.email_address AS registered_email_address ' .
                'FROM ' .
                    'user_company_invitations ' .
        ...
        // Optional Parameters
        //  None
        public function getStatusList($data, $user, $db) {
            // Set content type header
            header('Content-Type: application/json');
            ...
            $sqlQuery = 'SELECT code, name FROM user_company_invitation_status_types ORDER BY name ASC;';
            $sqlResult = $db->paramQuery($sqlQuery, []);
            if( !$sqlResult->isValid() ) {
                echo( json_encode(['ok' => false, 'error' => 'Database error.']) );
                return false;
